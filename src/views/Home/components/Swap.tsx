...
const Swap = () => {
  ...
  return (
    <div className="p-5 bg-[#fff] rounded-xl  mt-8 lg:mt-[150px] shadow-md mb-5">
      ...
      <div className="flex items-center justify-between text-xs lg:text-base text-[#333333] mt-8">
        <div>结算期</div>
...
const InputField: React.FC<{ symbol: string; className?: string }> = ({ symbol, className }) => {
  return (
    <div className={`bg-[#f0f0f0] p-5 rounded-xl ${className || ''}`}>
      <div className=" text-[#989898] text-xs lg:text-base">余额： 0.00000</div>
      <div className="flex items-center justify-between mt-2">
        <input type="number" className="flex-1 bg-transparent outline-none" />
        <div className="flex items-center gap-2">
          <img className="w-4 lg:w-[22px]" src={b} alt="" />
          <div className="text-[#292929] text-sm lg:text-xl">{symbol}</div>
        </div>
      </div>
    </div>
  )
